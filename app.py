...
def load_books():
    ...
    DEFAULT_IMAGE_LARGE = "https://via.placeholder.com/300x450?text=No+Cover"
    DEFAULT_IMAGE_SMALL = "https://via.placeholder.com/100x150?text=No+Cover"

    books_df["large_thumbnail"] = books_df["thumbnail"].apply(
        lambda url: (url + "&fife=w800") if pd.notna(url) and url.strip() != "" else DEFAULT_IMAGE_LARGE
    )

    books_df["small_thumbnail"] = books_df["thumbnail"].apply(
        lambda url: (url + "&fife=w200") if pd.notna(url) and url.strip() != "" else DEFAULT_IMAGE_SMALL
    )
...
def batch_embed(documents, embeddings, batch_size=100):
    ...
    if any(isinstance(v, float) for v in vectors):
        raise ValueError("Embedding result contains scalar floats; expected list of vectors.")

    if not vectors or len(texts) != len(vectors):
        raise ValueError("Mismatch between texts and embedding lengths.")

    text_vector_pairs = list(zip(texts, vectors))
    return FAISS.from_embeddings(text_vector_pairs, embeddings)
